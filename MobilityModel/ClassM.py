...
class ModelM(object):
    ...
    def read_data(self):
        ...
        print(self.DF_Gem.Gemeentenaam)
        ...
        # New mobility data
        df_mobFreq = pd.read_csv("../Mob_grav_freq_2019_pop_v3.csv").drop("Unnamed: 0",axis=1).to_numpy()
        df_mobInc = pd.read_csv("../Mob_grav_inc_2019_pop_v3.csv").drop("Unnamed: 0",axis=1).to_numpy()
        self.MobMat_freq = df_mobFreq
        self.MobMat_inc = df_mobInc
        ...
        self.DemoMat = (np.array(DemoMat_sorted)/self.Div).astype(int)
    ...
    def save(self, seed):
        ...
        pathSeed = os.path.join(path, self.SaveName + '/Seed_v3_' + str(seed) + '/')
        pathSeed = f"../Seed_v3_" + str(seed) + "/"

		...
		for demo_group in [demo]:
			...
			for run in range(runs_done, runs):
				params_input = {'savename': interv,
					...
					'respect_border_lockdown_probability': 0,
					'gamma': gamma
					}
				ClassT = ModelT(params_input)
				print(ClassT.big_city_threshold)
				ClassT.read_model_data(initial_loc, int(demo_group), n_initial_infect, no_group=True)
				ClassT.read_empirical_data()
				ClassT.set_parameters(latent, incub, infect)
				ClassT.initialise(demo_group)
				ClassT.simulate_new()
				status_file = ClassT.save(run, int(demo_group), initial_loc)
				del ClassT
			...
			# 	os.makedirs(path)
			# np.save(path + f'Status_avg_runs={runs}_{city_size}_{demo_group}', dense_average)
			# np.save(path + f'Status_std_runs={runs}_{city_size}_{demo_group}', dense_std)

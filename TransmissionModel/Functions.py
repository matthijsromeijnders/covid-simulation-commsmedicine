...
def determine_exposed(self, Stat, t, day, hour, phase):
    ...
    if self.Intervention == 'selfisolation':
        self_isolating = np.where((self.Incub_plus_self_isolation_lag + 10 > t) & (self.Incub_plus_self_isolation_lag < t))[0]
        
    else:
        self_isolating = []
    Ivec[self_isolating] = 0
    #print('SELF ISOLATING = ',len(self_isolating))
    Lvec = np.zeros(self.N)
    locs = np.zeros(355)
    Ipos = scipy.sparse.csr_matrix(self.PosMat[day, hour]) #positions of the infected?
    Is = scipy.sparse.csr_matrix((self.GroupsMat_sp.multiply(Ivec)).toarray()).T

    infs = (Ipos.dot(Is)).toarray() # infected agents
    tots = (Ipos.dot(self.GroupsMat_sp.T)).toarray() # total agents
    sucs = (Ipos.multiply(Svec)).toarray() # susceptible agents
    Lpos = np.zeros(self.N)
    ...
    fracs = infs/(1e-9+tots) # proportion of agents that are infected in a place
    ...
    for m in wh:
        ...
        if np.sum(fracs2) > 0:
            ...
            if self.Intervention == 'local' or self.Intervention == 'brablim' or self.Intervention == 'G4':
                beta = self.Betas[m]
            people = np.where(sucs[m] == 1)[0] # susceptible agents in a place
            for p in people:
    ...
    Lpos_exposed = Lpos[En]
    for pi_loc in Lpos_exposed:
        locs[int(pi_loc)] += 1
...
def force_of_infection2(self, p, fracs, m, hour, phase):
    ...
    return np.sum(mixvec*fracs*self.HG[group]) # contacts with infected people within muni * foi per contact, adjusted to demographic group
...
def close_muni_borders(self, prob):
    if True:
        closed = np.where(self.ClosedBorders == 1)[0]
        for m in closed:
            # Living now stay there
            # Get the indices where `self.HomesI` equals `m`
            wh = np.where(self.HomesI == m)[0]
            # Generate a random mask with a 90% chance of True (keep) for each element in `wh`
            keep_mask = np.random.rand(len(wh)) < prob
            # Select only 90% of the indices based on the keep mask
            wh_selected = wh[keep_mask]
            wh = wh_selected
            for d in range(7):
                for h in range(24):
                    self.PosMat[d, h, self.Positions[d, h, wh], wh] = 0
            ...
            for d in range(7):
                for h in range(24):
                    wh2 = np.where(self.Positions[d, h] == m)[0]
                    keep_mask = np.random.rand(len(wh1)) < prob
                    wh_selected = wh1[keep_mask]
                    wh1 = wh_selected
                    wh = np.intersect1d(wh1, wh2)
                    self.PosMat[d, h, self.Positions[d, h, wh], wh] = 0
                    self.Positions[d, h, wh] = self.HomesI[wh]
